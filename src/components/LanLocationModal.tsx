...
export const LanLocationModal: React.FC<LanLocationModalProps> = ({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!name.trim()) {
      setError('Nama Lokasi / Sekolah wajib diisi!');
      return;
    }
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="font-bold text-base text-slate-900 dark:text-slate-100">
                {editLocation ? 'Edit Lokasi / Sekolah' : 'Tambah Lokasi / Sekolah Baru'}
              </h3>
              <p className="text-xs text-slate-500 dark:text-slate-400">
                Tingkat 1: Induk tempat jaringan (misal: Sekolah 1, Sekolah 2, Kampus Utama)
              </p>
            </div>
          </div>
          <button
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <div className="sm:col-span-2 space-y-1.5">
              <label className="text-xs font-bold text-slate-700 dark:text-slate-300">
                Nama Lokasi / Sekolah <span className="text-rose-500">*</span>
              </label>
              <input
                type="text"
                placeholder="misal: SMK Negeri 1 (Sekolah 1)"
                value={name}
