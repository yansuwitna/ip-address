...
import { Volume2, Speaker as SpeakerIcon, 
  Plus, 
  Search, 
  Edit3, 
  Trash2, 
...
import { SoundDevice, SoundCableRun, SoundStatus, SoundDeviceType, LanLocation, LanZone } from '../types/jaringanUtilitas';
...
interface SoundViewProps {
  ...
  zones: LanZone[];
  devices: SoundDevice[];
  cables: SoundCableRun[];
  onSaveLocation?: (loc: Partial<LanLocation>) => void;
  onDeleteLocation?: (id: string) => void;
  onSaveZone?: (zone: Partial<LanZone>) => void;
  onDeleteZone?: (id: string) => void;
  onSaveDevice: (device: Partial<SoundDevice>) => void;
  onDeleteDevice: (id: string) => void;
  onSaveCable?: (cable: Partial<SoundCableRun>) => void;
  ...
  onOpenEditDeviceModal: (device: SoundDevice) => void;
  onOpenAddCableModal: (locationId?: string, zoneId?: string) => void;
  onOpenEditCableModal: (cable: SoundCableRun) => void;
...
export const SoundView: React.FC<SoundViewProps> = ({
  ...
  const locationZones = useMemo(() => {
    ...
    return zones.filter(z => z.locationId === selectedLocationId && z.systemType === 'sound');
  ...
  const stats = useMemo(() => {
    ...
    const online = contextDevices.filter(d => d.status === 'online' || false).length;
    const offlineOrIssue = contextDevices.filter(d => d.status === 'offline' || false).length;
  ...
  const filteredCables = useMemo(() => {
    return contextCables.filter(c => {
      const src = c.sourceLocation || '';
      const tgt = c.targetLocation || '';
      const matchSearch = 
  ...
  const getTypeLabel = (type: SoundDeviceType) => {
    switch (type) {
      case 'camera_ip_dome': return 'Dome Speaker (Indoor)';
      case 'camera_ip_bullet': return 'Bullet Speaker (Outdoor)';
      case 'camera_ip_ptz': return 'Speed Dome PTZ';
      case 'nvr': return 'NVR Recorder';
      case 'dvr': return 'DVR Analog';
      case 'switch_poe': return 'Switch PoE SOUND';
      case 'storage_nas': return 'NAS Storage';
      case 'monitor_matrix': return 'Monitor Matrix';
      default: return 'Perangkat SOUND';
    }
  };

  const handleDeleteLocationItem = async (loc: LanLocation) => {
    const locZones = zones.filter(z => z.locationId === loc.id && z.systemType === 'sound');
    ...
    if (locZones.length > 0 || locDevs.length > 0 || locCbls.length > 0) {
      const details = [];
      if (locZones.length > 0) details.push(`${locZones.length} area SOUND`);
      if (locDevs.length > 0) details.push(`${locDevs.length} speaker/NVR`);
      if (locCbls.length > 0) details.push(`${locCbls.length} jalur kabel SOUND`);
  ...
  const handleDeleteZoneItem = async (zone: LanZone) => {
    ...
    if (zoneDevs.length > 0 || zoneCbls.length > 0) {
      const details = [];
      if (zoneDevs.length > 0) details.push(`${zoneDevs.length} speaker/NVR`);
      if (zoneCbls.length > 0) details.push(`${zoneCbls.length} jalur kabel`);

      await showWarning(
        'Jaringan SOUND Tidak Dapat Dihapus!',
    ...
    if (confirmed && onDeleteZone) {
      ...
      showSuccess('Jaringan SOUND berhasil dihapus!');
    }
  };

  const handleDeleteDevice = async (dev: SoundDevice) => {
    const confirmed = await showConfirm({
      title: 'Hapus Perangkat SOUND?',
      text: `Perangkat "${dev.name}" (${dev.location}) akan dihapus permanen dari sistem.`,
      confirmButtonText: 'Ya, Hapus',
      cancelButtonText: 'Batal'
    });
    if (confirmed) {
      onDeleteDevice(dev.id);
      showSuccess('Perangkat SOUND berhasil dihapus!');
    }
  };

  const handleDeleteCable = async (cable: SoundCableRun) => {
    const confirmed = await showConfirm({
      title: 'Hapus Jalur Kabel SOUND?',
    ...
    if (confirmed && onDeleteCable) {
      ...
      showSuccess('Jalur kabel SOUND berhasil dihapus!');
    }
  };
  ...
  return (
    <div className="space-y-6 font-poppins animate-in fade-in duration-200">
          ...
          <div className="bg-slate-300 dark:bg-slate-800/95 border border-slate-400/80 dark:border-slate-700 rounded-3xl p-6 shadow-xs flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              <h2 className="text-xl font-extrabold text-slate-900 dark:text-slate-100 tracking-tight flex items-center gap-2.5">
                ...
                <span>Daftar Lokasi Jaringan SOUND</span>
              </h2>
              <p className="text-xs text-slate-600 dark:text-slate-300 mt-1">
                Pilih lokasi properti untuk melihat cakupan zona surveillance dan speaker SOUND di dalamnya
              </p>
            </div>
          ...
          {locations.length === 0 ? (
            <div className="bg-white dark:bg-slate-900 border border-slate-200/90 dark:border-slate-800 rounded-3xl p-12 text-center text-slate-400 space-y-3">
              ...
              <p className="text-xs max-w-md mx-auto">
                Mulai dengan menambahkan lokasi penempatan sistem speaker SOUND pertama Anda.
              </p>
              <button
                onClick={onOpenAddLocationModal}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
              {locations.map((loc) => {
                ...
                const onlineCam = locDevices.filter(d => d.status === 'online' || false).length;
      ...
      {/* TINGKAT 2: DAFTAR JARINGAN SOUND DI LOKASI TERPILIH */}
        ...
        <div className="space-y-6">
          <div className="bg-slate-300 dark:bg-slate-800/95 border border-slate-400/80 dark:border-slate-700 rounded-3xl p-6 shadow-xs flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              ...
              <h2 className="text-xl font-extrabold text-slate-900 dark:text-slate-100 tracking-tight flex items-center gap-2.5">
                ...
                <span>Daftar Jaringan SOUND - {activeLocation.name}</span>
              </h2>
              <p className="text-xs text-slate-600 dark:text-slate-300 mt-1">
                Pilih atau tambah jaringan SOUND (misal: Area Parkir & Gerbang, Area Koridor & Lobi)
              </p>
            </div>
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2.5 w-full sm:w-auto">
              <button
          ...
          {locationZones.length === 0 ? (
            <div className="bg-white dark:bg-slate-900 border border-slate-200/90 dark:border-slate-800 rounded-3xl p-12 text-center text-slate-400 space-y-3">
              ...
              <h4 className="text-base font-bold text-slate-700 dark:text-slate-300">Belum ada Jaringan SOUND di {activeLocation.name}</h4>
              <p className="text-xs max-w-md mx-auto">
                Buat nama jaringan SOUND pertama (misalnya: Jaringan SOUND Perimeter, SOUND Lantai 1, SOUND Ruang Server).
              </p>
              <button
                onClick={() => onOpenAddZoneModal(activeLocation.id)}
                className="mt-2 px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-bold rounded-xl shadow-md shadow-indigo-600/25 transition-all cursor-pointer inline-flex items-center gap-1.5"
                    ...
                    <div>
                      ...
                      <div className="grid grid-cols-2 gap-2 mt-4 pt-3 border-t border-slate-100 dark:border-slate-800 text-center">
                        <div className="p-2 rounded-xl bg-slate-50 dark:bg-slate-800/60">
                          ...
                          <span className="block text-[10px] text-slate-400">Speaker / NVR</span>
                        </div>
                        <div className="p-2 rounded-xl bg-slate-50 dark:bg-slate-800/60">
                          <span className="block text-[11px] font-bold text-slate-700 dark:text-slate-200">
                            {zoneCables.length}
                          </span>
                          <span className="block text-[10px] text-slate-400">Jalur Kabel SOUND</span>
                        </div>
                      </div>
                    </div>
      ...
      {/* TINGKAT 3: DETAIL JARINGAN (KAMERA/NVR & JALUR KABEL SOUND) */}
        ...
        <div className="space-y-6">
          <div className="bg-slate-300 dark:bg-slate-800/95 border border-slate-400/80 dark:border-slate-700 rounded-3xl p-6 shadow-xs flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              ...
              <p className="text-xs text-slate-600 dark:text-slate-300 mt-1">
                Pantau speaker IP, perekam NVR/DVR, dan rute kabel transmisi PoE surveillance
              </p>
            </div>
            ...
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2.5 w-full sm:w-auto">
              ...
              {/* Tombol Simulasi Topologi SOUND & Jalur Sinyal */}
              {activeLocation && activeZone && (
                <button
                  ...
                  title="Buka Simulasi Topologi SOUND & Jalur Sinyal"
                >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <div className="bg-white dark:bg-slate-900 border border-slate-200/90 dark:border-slate-800 rounded-3xl p-5 shadow-xs relative overflow-hidden">
              ...
              <p className="text-[11px] text-slate-500 mt-2">
                <span className="font-bold text-slate-700 dark:text-slate-300">{stats.cameras}</span> Speaker • <span className="font-bold text-slate-700 dark:text-slate-300">{stats.nvrCount}</span> NVR/DVR
              </p>
            </div>
            ...
            <div className="bg-white dark:bg-slate-900 border border-slate-200/90 dark:border-slate-800 rounded-3xl p-5 shadow-xs relative overflow-hidden">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-xs font-semibold text-slate-500 dark:text-slate-400">Speaker Online</p>
            ...
            <div className="bg-white dark:bg-slate-900 border border-slate-200/90 dark:border-slate-800 rounded-3xl p-5 shadow-xs relative overflow-hidden">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-xs font-semibold text-slate-500 dark:text-slate-400">Jalur Kabel SOUND</p>
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 border-b border-slate-200 dark:border-slate-800 pb-2">
            <div className="flex items-center gap-2 overflow-x-auto">
              ...
              >
                ...
                <span>Perangkat SOUND & NVR ({contextDevices.length})</span>
              </button>
              <button
                onClick={() => {
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200/90 dark:border-slate-800 rounded-2xl p-4 shadow-xs flex flex-col md:flex-row gap-3 items-center justify-between">
            <div className="relative w-full md:w-80">
              ...
              <input
                ...
                placeholder={activeSubTab === 'devices' ? "Cari nama speaker, IP address, lokasi..." : "Cari label kabel, titik asal, tujuan..."}
            ...
            <div className="flex items-center gap-2.5 w-full md:w-auto">
              {activeSubTab === 'devices' && (
                ...
                >
                  <option value="all">Semua Tipe SOUND</option>
                  <option value="camera_ip_dome">Dome Speaker (Indoor)</option>
                  <option value="camera_ip_bullet">Bullet Speaker (Outdoor)</option>
                  <option value="camera_ip_ptz">Speed Dome PTZ</option>
                  <option value="nvr">NVR Recorder</option>
                  <option value="dvr">DVR Analog</option>
                  <option value="switch_poe">Switch PoE SOUND</option>
                </select>
              )}
          ...
          {/* TAB 1: Perangkat Speaker & Perekam */}
          {activeSubTab === 'devices' && (
            <div>
              {filteredDevices.length === 0 ? (
                <div className="bg-white dark:bg-slate-900 border border-slate-200/90 dark:border-slate-800 rounded-3xl p-12 text-center text-slate-400 space-y-3">
                  <Volume2 className="w-12 h-12 mx-auto text-slate-300 dark:text-slate-600 opacity-60" />
                  <h4 className="text-sm font-bold text-slate-700 dark:text-slate-300">Tidak ada perangkat ditemukan</h4>
                  <p className="text-xs">
                    {searchQuery ? 'Coba ubah kata kunci pencarian Anda.' : 'Belum ada perangkat SOUND di jaringan ini. Klik tombol di atas untuk menambah.'}
                  </p>
                </div>
              ) : (
          ...
          {/* TAB 2: Pencatatan Jalur Kabel SOUND */}
          {activeSubTab === 'cables' && (
            <div>
              {filteredCables.length === 0 ? (
                <div className="bg-white dark:bg-slate-900 border border-slate-200/90 dark:border-slate-800 rounded-3xl p-12 text-center text-slate-400 space-y-3">
                  <Route className="w-12 h-12 mx-auto text-slate-300 dark:text-slate-600 opacity-60" />
                  <h4 className="text-sm font-bold text-slate-700 dark:text-slate-300">Belum ada jalur kabel SOUND</h4>
                  <p className="text-xs">
                    Catat rute kabel dari Switch PoE / NVR ke speaker surveillance.
                  </p>
                  <button
                    onClick={() => onOpenAddCableModal(activeLocation.id, activeZone.id)}
                    className="mt-2 px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-bold rounded-xl shadow-md shadow-indigo-600/25 transition-all cursor-pointer inline-flex items-center gap-1.5"
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {filteredCables.map((cable) => {
                    const src = cable.sourceLocation || '';
                    const tgt = cable.targetLocation || '';
                    const len = cable.lengthMeter || 0;
                      ...
                      >
                        <div>
                          ...
                          <div className="mt-4 p-3 rounded-2xl bg-slate-50 dark:bg-slate-800/60 border border-slate-100 dark:border-slate-800 space-y-2 text-xs">
                            ...
                            <div>
                              <span className="text-[10px] text-slate-400 block">Titik Tujuan (Speaker SOUND):</span>
