...
import { X, Route, MapPin, Layers, ArrowRight, ShieldCheck, Tag, Lock } from 'lucide-react';
import { SoundCableRun, SoundDevice, LanLocation, LanZone, SoundCableTypeItem } from '../types/jaringanUtilitas';
import { ModalPortal } from './ModalPortal';

interface SoundCableModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (cable: Partial<SoundCableRun>) => void;
  editCable?: SoundCableRun | null;
  locations: LanLocation[];
  zones: LanZone[];
  soundDevices: SoundDevice[];
  cableTypes?: SoundCableTypeItem[];
  presetLocationId?: string;
  presetZoneId?: string;
}

export const SoundCableModal: React.FC<SoundCableModalProps> = ({
  isOpen,
  onClose,
  onSave,
  editCable,
  ...
  const [formData, setFormData] = useState<Partial<SoundCableRun>>({
    cableCode: '',
  ...
  useEffect(() => {
    if (editCable) {
      setFormData({
        ...editCable,
        sourceLocation: editCable.sourceLocation || editCable.sourceLocation,
        targetLocation: editCable.targetLocation || editCable.targetLocation,
      });
      setSelectedLocationId(editCable.locationId || presetLocationId || '');
      setSelectedZoneId(editCable.zoneId || presetZoneId || '');
    } else {
      setFormData({
        cableCode: `CBL-SOUND-${Math.floor(100 + Math.random() * 900)}`,
        cableType: 'Cat6 UTP (PoE)',
        sourceLocation: '',
        targetLocation: '',
        lengthMeter: 25,
  ...
  const availableZones = zones.filter(z => z.systemType === 'sound' && (!selectedLocationId || z.locationId === selectedLocationId));

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    const src = formData.sourceLocation || formData.sourceLocation || '';
    const tgt = formData.targetLocation || formData.targetLocation || '';
    if (!formData.cableCode || !src || !tgt) return;

    const cableToSave: SoundCableRun = {
      id: editCable?.id || `sound-cable-${Date.now()}`,
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs font-poppins animate-in fade-in duration-150">
      <div className="relative w-full max-w-4xl mx-auto bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 rounded-2xl shadow-2xl overflow-hidden max-h-dvh flex flex-col animate-in zoom-in-95 duration-150">
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h3 className="text-xl font-bold text-slate-800 dark:text-white">
                {editCable ? 'Edit Jalur Kabel SOUND' : 'Catat Jalur Kabel SOUND'}
              </h3>
              <p className="text-xs text-slate-500 dark:text-slate-400">
                Pencatatan rute sinyal dan daya SOUND (PoE Cat6 / Coaxial)
              </p>
            </div>
          </div>
          <button
        ...
        <div className="p-6 overflow-y-auto space-y-4">
          <form id="sound-cable-form" onSubmit={handleSubmit} className="space-y-4">
            {/* Lokasi & Jaringan */}
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => {
                    ...
                    const matching = zones.filter(z => z.systemType === 'sound' && z.locationId === e.target.value);
                    setSelectedZoneId(matching[0]?.id || '');
                  }}
                  disabled={true}
                  className="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-100 dark:bg-slate-800/80 text-slate-500 dark:text-slate-400 text-sm cursor-not-allowed select-none opacity-80"
                >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  placeholder="Contoh: CBL-SOUND-01"
              ...
              <div>
                ...
                >
                  {cableTypes && cableTypes.length > 0 ? (
                    cableTypes.map(ct => (
                      <option key={ct.id} value={ct.name || ct.code}>
                        {ct.name}
                      </option>
                    ))
                  ) : (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <input
                  type="text"
                  required
                  value={formData.sourceLocation || formData.sourceLocation || ''}
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1 flex items-center gap-1.5">
                  ...
                  Titik Tujuan (Speaker SOUND) *
                </label>
                <input
                  type="text"
                  required
                  value={formData.targetLocation || formData.targetLocation || ''}
                  onChange={(e) => setFormData({ ...formData, targetLocation: e.target.value })}
                  placeholder="Contoh: SOUND Dome Lobby Depan"
                  className="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-800 dark:text-white text-sm focus:ring-2 focus:ring-indigo-500 outline-hidden"
                />
              </div>
            </div>
