...
export const GroupList: React.FC<GroupListProps> = ({
  ...
  return (
    <div className="bg-white dark:bg-slate-900 border border-slate-200/90 dark:border-slate-800 rounded-2xl overflow-hidden shadow-xs flex flex-col h-full">
      ...
      <div className="p-2 border-b border-slate-100 bg-slate-50 dark:bg-slate-800/50/50">
        <button
          onClick={() => onSelectGroup(null)}
          className={`w-full text-left px-3 py-2 rounded-xl text-xs transition-all flex items-center justify-between cursor-pointer ${
            selectedGroupId === null
              ? 'bg-blue-600 text-white font-semibold shadow-sm'
              : 'text-slate-600 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-700/70 hover:text-slate-900 dark:hover:text-slate-100 dark:text-slate-100 font-medium'
          }`}
        >
          ...
          <span className={`text-[11px] px-2 py-0.5 rounded-full ${
            selectedGroupId === null ? 'bg-blue-700/80 text-white' : 'bg-slate-200 text-slate-600 dark:text-slate-400'
          }`}>
              ...
              <div
                ...
                className={`group relative p-3 rounded-xl cursor-pointer transition-all border ${
                  isSelected
                    ...
                    : 'bg-white dark:bg-slate-900 border-slate-200/80 hover:bg-slate-50 dark:hover:bg-slate-800/80 dark:bg-slate-800/50 hover:border-slate-300 dark:border-slate-600'
                }`}
              >
                {/* Color Pill */}
                <div 
                  ...
                  <div className="flex items-start justify-between gap-1">
                    ...
                    <div className="flex items-center space-x-0.5 opacity-80 group-hover:opacity-100">
                      <button
                        ...
                        className="p-1 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-400 hover:text-slate-700 dark:text-slate-300 rounded transition-colors"
                      >
