...
interface LanRoomTypesViewProps {
  ...
  onSaveRoomType: (item: LanRoomTypeItem) => void;
  onDeleteRoomType: (id: string) => void;
}

export const LanRoomTypesView: React.FC<LanRoomTypesViewProps> = ({
  roomTypes = [],
  onSaveRoomType,
  onDeleteRoomType
}) => {
  ...
  const [editingItem, setEditingItem] = useState<LanRoomTypeItem | null>(null);
  ...
  const openEditModal = (item: LanRoomTypeItem) => {
    setEditingItem(item);
    setName(item.name);
    setCode(item.code);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!name.trim()) {
      setError('Nama tipe ruangan wajib diisi!');
      return;
    }
    const cleanCode = code.trim() || name.toLowerCase().replace(/[^a-z0-9]+/g, '_');
    if (!editingItem && roomTypes.some(r => r.code.toLowerCase() === cleanCode.toLowerCase())) {
      setError(`Kode / slug "${cleanCode}" sudah digunakan oleh tipe ruangan lain!`);
      return;
    }

    onSaveRoomType({
      id: editingItem ? editingItem.id : `rt-${Date.now()}`,
      name: name.trim(),
      code: cleanCode,
  ...
  const filteredItems = roomTypes.filter(r => {
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200/90 dark:border-slate-800 rounded-3xl p-6 shadow-xs flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-xl font-extrabold text-slate-900 dark:text-slate-100 tracking-tight flex items-center gap-2.5">
            <span>Master Tipe Ruangan / Lab</span>
            <span className="text-xs font-bold px-2.5 py-0.5 rounded-full bg-emerald-50 dark:bg-emerald-900/40 text-emerald-700 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-800">
              {roomTypes.length} Tipe Ruangan
            </span>
          </h2>
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
            Kelola pilihan tipe ruangan yang digunakan saat membuat Jaringan / Sub-Sistem baru.
          </p>
        </div>

        <button
          onClick={openAddModal}
          className="flex items-center gap-2 px-4 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-xs font-bold shadow-sm shadow-emerald-600/30 transition-all cursor-pointer w-full sm:w-auto justify-center"
        >
          ...
          <table className="w-full text-left text-xs border-collapse">
            <thead>
              <tr className="bg-slate-50 dark:bg-slate-800/80 border-b border-slate-200 dark:border-slate-700 text-slate-500 dark:text-slate-400 font-bold uppercase text-[10px] tracking-wider">
                ...
                <th className="py-3.5 px-4">Nama Tipe Ruangan</th>
                ...
                <tr>
                  <td colSpan={5} className="py-14 text-center text-slate-400 text-xs">
                    <div className="flex flex-col items-center justify-center gap-2">
                      ...
                      <p className="font-semibold text-slate-700 dark:text-slate-300">Belum ada Tipe Ruangan.</p>
                      <p className="text-[11px] text-slate-400">Tambahkan tipe ruangan pertama agar dapat dipilih saat Tambah Jaringan / Sub-Sistem Baru.</p>
                      <button
                        onClick={openAddModal}
                        className="mt-2 px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-xs font-bold shadow-xs transition-all cursor-pointer"
                      >
                        + Tambah Tipe Ruangan Sekarang
                      </button>
                    </div>
                  </td>
                </tr>
                  ...
                  <tr key={item.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/60 transition-colors">
                    ...
                    <td className="py-3.5 px-4 font-bold text-slate-900 dark:text-slate-100">
                      <div className="flex items-center gap-2.5">
                        <div className="p-2 rounded-xl bg-emerald-50 dark:bg-emerald-900/40 text-emerald-600 dark:text-emerald-400 border border-emerald-100 dark:border-emerald-800/60 flex-shrink-0">
                        ...
                        <button
                          onClick={async () => {
                            const confirmed = await showConfirm({
                              ...
                              text: `Apakah Anda yakin ingin menghapus tipe ruangan "${item.name}"?`,
                              confirmButtonText: 'Ya, Hapus',
                              cancelButtonText: 'Batal',
                              isDanger: true
                            });
                            if (confirmed) {
                              onDeleteRoomType(item.id);
                              showSuccess('Tipe Ruangan Dihapus', `Tipe ruangan "${item.name}" berhasil dihapus.`);
                            }
                          }}
                          title="Hapus Tipe Ruangan"
              ...
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h3 className="font-bold text-slate-900 dark:text-slate-100 text-sm">
                    {editingItem ? 'Edit Tipe Ruangan' : 'Tambah Tipe Ruangan Baru'}
                  </h3>
                  <p className="text-[11px] text-slate-400">
                    Opsi dropdown Tipe Ruangan pada form Tambah Jaringan / Sub-Sistem.
                  </p>
                </div>
              </div>
              <button
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div>
                <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1">
                  Nama Tipe Ruangan <span className="text-rose-500">*</span>
                </label>
                <input
                  type="text"
                  required
                  ...
                  placeholder="Contoh: Laboratorium Komputer, Ruang Server, Ruang Guru"
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="Contoh: lab, server_room, office"
                  className={`w-full px-3.5 py-2.5 border rounded-xl text-xs font-mono transition-all ${
                    editingItem 
                      ...
                      : 'bg-slate-50 dark:bg-slate-800/40 hover:bg-slate-100 dark:hover:bg-slate-800/70 focus:bg-white dark:focus:bg-slate-900 border-slate-200 dark:border-slate-700 text-slate-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-emerald-500'
                  }`}
                />
                <p className="text-[10px] text-slate-400 mt-1">
                  Nilai internal unik (slug huruf kecil) untuk tipe ruangan.
                </p>
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full px-3.5 py-2 bg-slate-50 dark:bg-slate-800/40 hover:bg-slate-100 dark:hover:bg-slate-800/70 focus:bg-white dark:focus:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl text-xs text-slate-900 dark:text-slate-100 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-emerald-500 transition-all resize-none"
                />
              </div>
              ...
              <div className="pt-3 border-t border-slate-100 dark:border-slate-800 flex flex-col-reverse sm:flex-row items-stretch sm:items-center justify-end gap-2.5 w-full">
                ...
                >
                  {editingItem ? 'Simpan Perubahan' : 'Tambah Tipe Ruangan'}
                </button>
              </div>
            </form>
          </div>
