...
import { ArrowLeft, Plus, Search, Edit2, Trash2, Globe, Check, Database, Folder, Hash, Server, ExternalLink, CornerDownRight } from 'lucide-react';
...
export const SubDomainView: React.FC<SubDomainViewProps> = ({ 
  ...
  const filteredSubs = parentSubs
    .filter(s => {
      if (!search.trim()) return true;
      const q = search.toLowerCase();
      return s.subName.toLowerCase().includes(q) || 
             (s.ipAddress || '').toLowerCase().includes(q) ||
             (s.forward || '').toLowerCase().includes(q) ||
             (s.folder || '').toLowerCase().includes(q) ||
             (s.database || '').toLowerCase().includes(q) ||
             (s.description || '').toLowerCase().includes(q);
    })
    .sort((a, b) => a.subName.localeCompare(b.subName, undefined, { numeric: true, sensitivity: 'base' }));
  ...
  const submitModal = (e: React.FormEvent) => {
    ...
    if (editingSub) {
      handleSave(subDomains.map(s => s.id === editingSub.id ? {
        ...
        folder,
        database,
        forward: forward.trim() || undefined,
        description
      } : s));
    } else {
                ...
                filteredSubs.map(item => (
                  <tr key={item.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/20 transition-colors">
                    <td className="py-3.5 px-4 whitespace-nowrap font-mono font-bold text-slate-900 dark:text-slate-100">
                      ...
                      {item.forward && (
                        <div className="flex items-center gap-1 text-[11px] text-amber-600 dark:text-amber-400 font-normal mt-0.5" title={`Forward ke: ${item.forward}`}>
                          <CornerDownRight className="w-3 h-3 flex-shrink-0" />
                          <span className="truncate max-w-[200px]">{item.forward}</span>
                        </div>
                      )}
                    </td>
                    ...
                    <td className="py-3.5 px-4 text-right whitespace-nowrap">
                      <div className="flex items-center justify-end space-x-1">
                        <a
                          href={`${item.protocol || 'http'}://${item.subName}.${parentDomain.domain}`}
                          target="_blank"
                          rel="noopener noreferrer"
                          title={`Buka ${item.protocol || 'http'}://${item.subName}.${parentDomain.domain} di tab baru`}
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-3xl w-full max-w-lg shadow-2xl overflow-hidden my-8 animate-in fade-in zoom-in-95 duration-150">
            ...
            <form onSubmit={submitModal} className="p-6 space-y-5">
              ...
              <div className="space-y-4">
                ...
                <div>
                  <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1.5">
                    Forward <span className="text-[11px] font-normal text-slate-400">(Opsional)</span>
                  </label>
                  <div className="relative">
                    <div className="absolute left-3.5 top-1/2 -translate-y-1/2">
                    ...
                    <input
                      ...
                      placeholder="Contoh: https://layanan-lain.local:8443 atau 192.168.1.100:8080"
                      className="w-full pl-10 pr-4 py-2.5 bg-slate-50 dark:bg-slate-800/40 border border-slate-200 dark:border-slate-700 rounded-xl text-sm text-slate-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all font-mono"
                    />
                  </div>
                </div>
